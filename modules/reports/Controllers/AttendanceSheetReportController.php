...
class AttendanceSheetReportController extends Controller
{
    ...
    public function generate_attendance_sheet_report(Request $request){
        ...
        $typing_model = DB::table( 'user AS u' )
         ->select('u.id','u.sl','u.roll_no','u.username','u.middle_name','u.last_name','u.started_exam','u.attended_typing_test','u.attended_aptitude_test','u.typing_status','u.aptitude_status','e.exam_type','e.company_id','e.designation_id','e.exam_date','c.company_name','c.address','d.designation_name')
    ...
    if ($values->exam_type == 'aptitude_test') {
        ...
        $expelled = $model_collection->filter(function ($value) {
            return $value->presence == "Expelled";
        })->sortBy(function ($values, $key) {
            return $values->sl;
        });

        $cancelled = $model_collection->filter(function ($value) {
            return $value->presence == "Cancelled";
        })->sortBy(function ($values, $key) {
            return $values->sl;
        });


        $model_collection = $present->merge($absent)->merge($expelled)->merge($cancelled);
